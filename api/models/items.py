...
class Items(Base):
    ...
    def remove_item(self, item_id, dry_run=False):
        ...
        if item_to_remove:
            if dry_run:
                return {
                    "message": f"Item with UID {item_id} would be removed (dry-run mode)."
                }, 200
            else:
                ...
                self.load(
                    is_debug=False
                )  # Reload data from the file to reflect the changes
                return {
                    "message": f"Item with UID {item_id} successfully removed."
                }, 200
        else:
    ...
    def load(self, is_debug=False):
        ...
        if is_debug:
            ...
            try:
                ...
                print(
                    f"Error decoding JSON from {self.data_path}. Initializing empty data."
                )
                self.data = []
